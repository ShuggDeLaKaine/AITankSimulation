...
public abstract class AITank : MonoBehaviour
{
    ...
    protected float fuelMax = 125f;
    protected int ammo;
    protected int ammoMax = 15;
    protected float health = 100f;
    protected float healthMax = 125f;
    ...
    private Dictionary<GameObject, float> ammoFound = new Dictionary<GameObject, float>();
    private Dictionary<GameObject, float> fuelFound = new Dictionary<GameObject, float>();
    private Dictionary<GameObject, float> healthFound = new Dictionary<GameObject, float>();
            ...
            if (Vector3.Angle(sensorPoint.transform.forward, directionToTarget) < viewAngle / 2)
            {
                ...
                if (!Physics.Raycast(sensorPoint.transform.position, directionToTarget, distanceToTarget, obstacleMask))
                {
                    if (target != this.gameObject && !consumablesFound.ContainsKey(target))
                    {
                        if (target.tag == "Ammo")
                        {
                            ammoFound.Add(target, distanceToTarget);
                        } else if (target.tag == "Fuel")
                        {
                            fuelFound.Add(target, distanceToTarget);
                        } else if (target.tag == "Health")
                        {
                            healthFound.Add(target, distanceToTarget);
                        }
    ...
    public Dictionary<GameObject, float> GetAmmoFound
    {
        get
        {
            return ammoFound;
    ...
    public Dictionary<GameObject, float> GetFuelFound
    {
        get
        {
            return fuelFound;
    ...
    public Dictionary<GameObject, float> GetHealthFound
    {
        get
        {
            return healthFound;
